...
def run_script(args):
    ...
    refine_script = os.path.join(script_dir, "refine_ID.py")
    cmd = f"CUDA_VISIBLE_DEVICES={gpu_id} python {refine_script} --index {index} --total {total} --video_list_path {video_list_path} --output {output}"
    print(cmd)
