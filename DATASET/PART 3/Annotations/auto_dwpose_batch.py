...
import argparse
def run_script(args):
    index, gpu_id, total, video_list_path,save_dir = args
    script_dir = os.path.dirname(__file__)
    single_dwpose = os.path.join(script_dir, "auto_single_dwpose.py")
    cmd = f"CUDA_VISIBLE_DEVICES={gpu_id} python {single_dwpose} --index {index} --total {total} --video_list_path {video_list_path} --save_dir {save_dir}"
    print(cmd)
...
def get_parse():
    ...
    parser.add_argument("--save_dir", type=str, default="./dwpose_result")
...
def main():
    ...
    num_cur = opt.num_cur
    tasks = [(i + shift, i % num_GPU, num_Total,video_list_path,save_dir) for i in range(num_cur)] 
